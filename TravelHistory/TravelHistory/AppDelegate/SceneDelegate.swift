...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneDidDisconnect(_ scene: UIScene) {
        ...
        if LocationTracker.shared.currentLocation != nil {
            LocationTracker.shared.useAbleLocation(newLocation: LocationTracker.shared.currentLocation!)
        }
    }
    ...
    func sceneWillEnterForeground(_ scene: UIScene) {
        ...
        if let homevc = window?.rootViewController as? HomeVC{
            LocationTracker.shared.getSavedLocations(limit: 100, ascending: true) { [self] (totalList) in
                homevc.locationList = totalList
                homevc.locationListTableView.reloadData()
                bgtimer?.invalidate()
                LocationTracker.shared.isBackGround = false
            }
        }
    ...
    @objc func bgtimer(_ timer:Timer!){
        LocationTracker.shared.startLocationTracker()
        LocationTracker.shared.saveInBackGround()
    }
